...
exports.unfriend = asyncHandler(async(req, res) => {
    ...
    if(index){res.status(401).send("Already unfriended.")}
...
exports.userDetails = asyncHandler(async(req, res) => {
    if(req.params.id==req.user._id){
        ...
        const user = await User.findById(req.params.id, {first_name: 1, last_name:1, picture:1, cover:1, friends:1}).populate("friends", "first_name last_name picture")
        res.json(user)
    }else{
        const user = await User.findById(req.params.id, {first_name: 1, last_name:1, picture:1, cover:1, friends:1}).populate("friends", "first_name last_name picture")
        res.json(user)
